...
export default function ServiceCard({ icon: Icon, title, description, delay = 0, variant = 'blue', href = null }) {
  const colors = {
    blue: {
  ...
  const card = (
    <motion.div
      ...
      whileHover={{ y: -5 }}
      className={`
        h-full p-4 sm:p-6 rounded-xl sm:rounded-2xl border-2 border-gray-100 dark:border-gray-800
        bg-white dark:bg-gray-800/50
        ${colorScheme.hover}
        transition-all duration-300 ${href ? 'cursor-pointer' : 'cursor-default'}
      `}
    >
      ...
      {href && (
        <span className={`inline-block mt-3 text-xs sm:text-sm font-semibold ${colorScheme.link}`}>
          Learn More →
        </span>
      )}
    </motion.div>
