...
const OUR_NUMBER = toE164(PHONE.e164)

// Car2Fix advertises one line for both shops, so the dialed number no longer
// says which shop the caller wants. What does tell us is whose extension the
// feed belongs to: RINGCENTRAL_JWT is minted by the Body Shop user, and without
// company-wide call-log rights we only ever see that extension's calls (see
// syncCallLog's extension fallback). So every call ingested today is a body-shop
// call. When the mechanical line is added this has to become a per-extension or
// per-dialed-number mapping rather than one constant. The Lead's own shopType is
// still what gets corrected in the CRM once someone talks to the customer.
export const CALL_SHOP_TYPE = 'body'
...
function isOwnNumber(e164) {
  return e164 != null && e164 === OUR_NUMBER
}
